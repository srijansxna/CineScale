...
class VideoService:
    ...
    async def delete_video(self, video_id: str) -> bool:
        """Delete a video and its associated job from the database."""
        video = await self.get_video_by_id(video_id)
        if not video:
            return False
        # Delete associated job first
        job = await self.get_job_by_video_id(video_id)
        if job:
            await self.db.delete(job)
    ...
    async def toggle_pin(self, video_id: str) -> Video | None:
        """Toggle the is_pinned flag on a video."""
        video = await self.get_video_by_id(video_id)
        if not video:
            return None
    ...
    async def update_thumbnail_config(
        ...
        """Persist thumbnail selection and title overlay result."""
        video = await self.get_video_by_id(video_id)
        if not video:
            return None
        video.video_title = video_title
        video.default_thumbnail = default_thumbnail
        video.final_thumbnail_path = final_thumbnail_path
        await self.db.commit()
        await self.db.refresh(video)
        return video
