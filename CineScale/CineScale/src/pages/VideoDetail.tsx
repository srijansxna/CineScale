...
import { Button, StatusBadge, ThumbnailSelector, VideoDetailSkeleton } from '../components'
...
// Use a relative path for media so it goes through the Vite proxy (avoids
// cross-origin range-request issues with the browser <video> element).
function mediaUrl(path: string): string {
  // If path already starts with http (absolute), strip the origin so the
  // browser request goes through the dev-server proxy instead.
...
export default function VideoDetail() {
  ...
  // Use relative paths so requests go through the Vite proxy — fixes cross-origin
  // range-request failures that cause the <video> element to show a black screen.
  const videoSrc = currentVariant?.url ? mediaUrl(currentVariant.url) : undefined
  const posterSrc = video.final_thumbnail_url
    ? `${API}${video.final_thumbnail_url}`
    : video.thumbnails[1]
      ? `${API}${video.thumbnails[1]}`
      : undefined

  const isDone = video.processing.status === 'DONE'
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
        <div>
          ...
          {video.video_title && (
            <p className="text-zinc-500 text-xs mt-0.5 font-mono">{video.filename}</p>
          )}
      ...
      {isDone && video.thumbnails.length > 0 && (
        <motion.div
          variants={slideUp}
          initial="hidden"
          animate="visible"
        ...
        >
          <ThumbnailSelector
            ...
            initialTitle={video.video_title}
            initialDefaultThumb={video.default_thumbnail}
            onSaved={() => queryClient.invalidateQueries({ queryKey: ['video', videoId] })}
          />
        </motion.div>
      )}

      <Link to="/dashboard" className="text-zinc-500 text-sm hover:text-white transition-colors">
        Back to dashboard
      </Link>
    </motion.div>
  )
